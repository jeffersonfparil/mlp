...
function prep_agridat_data(ARGS::Vector{String})::Nothing
    ...
    if (length(explanatory_names) == 0) || (length(response_names) == 0)
        ...
        for y_name in response_names
            ...
            idx = findall(.!ismissing.(df[!, y_name]) .&& .!isnan.(df[!, y_name]) .&& .!isinf.(df[!, y_name]))
            df_sub = select(df, vcat([y_name], explanatory_names))[idx, :]
...
precompile(prep_agridat_data, Vector{String})
