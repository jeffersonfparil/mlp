...
mod tests {
    ...
    fn test_marginal() -> Result<(), Box<dyn Error>> {
        ...
        marginals.effects.iter().zip(vec![-0.8072873, -0.8094203, -0.816374, -0.79108906, -0.78428054, -0.80378586, -0.8110637].iter()).for_each(|(a, b)| {assert_relative_eq!(a, b, epsilon=1.0e-6)});
        ...
        let seed: usize = 123;
        let mut marginals = Marginals::new(data.feature_names.clone(), 1)?;
        marginals.estimate_deepshap(&mut network, 100, seed, false)?;
